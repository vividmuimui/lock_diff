...
module LockDiff
  module Gem
    RSpec.describe Spec do
      ...
      let(:spec_with_revision) { described_class.new(test_data.sample_git_source) }
      let(:spec_with_git_tag) { described_class.new(test_data.lazy_specifications.find { |a| a.name == "brakeman" }) }
      let(:spec_without_ref) { described_class.new(test_data.lazy_specifications.find { |a| a.name == "dummy_gem" }) }
      let(:null_spec) { NullSpec.new("dummy") }

      describe "#ref", :with_http do
        specify do
