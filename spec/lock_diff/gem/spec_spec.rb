    ...
    RSpec.describe Spec do
      ...
      describe ".new" do
        it "returns RubyGemSpec if source is rubygems" do
          ...
          expect(described_class.new(lazy_specification)).to be_a Spec::RubyGemSpec
        end

        it "returns RubyGemSpec if source is git" do
          lazy_specification = test_data.sample_git_source
          expect(described_class.new(lazy_specification)).to be_a Spec::GitSpec
        end

        it "returns RubyGemSpec if source is path" do
          lazy_specification = test_data.sample_path_source
          expect(described_class.new(lazy_specification)).to be_a Spec::PathSpec
        end

        it "raises error if unknown source" do
          lazy_specification = double(source: :unknown, name: :unknown)
          expect { described_class.new(lazy_specification) }.to raise_error Spec::UnSupportSource
        end
      end
      ...
      describe "RubyGemSpec class" do
        ...
        describe "#repository_url", :with_http do
          specify do
            VCR.use_cassette("github.com") do
              expect(instance.repository_url).to start_with "https://github.com"
            end
          end
        end

        describe "#ruby_gem_url", :with_http do
          specify do
            VCR.use_cassette("rubygems.org") do
              expect(instance.ruby_gem_url).to start_with "https://rubygems.org/gems/"
            end
          end
        end
      end
      ...
      describe "GitSpec class" do
        ...
        describe "#repository_url", :with_http do
          specify { expect(instance.repository_url).to start_with "https://github.com/" }
        end
      end
